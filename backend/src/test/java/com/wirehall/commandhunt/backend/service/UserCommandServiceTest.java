...
class UserCommandServiceTest {
  ...
  void should_GetAllUserCommands_When_FilterAndUserEmailProvided() {
    ...
    when(userCommandRepository.findAll(isA(Specification.class), isA(Pageable.class)))
        .thenReturn(ucePage);

    // When
    PageResponse<UserCommand> pageResponse =
        userCommandService.getAllUserCommands(filter, "abc@xyz.com");

    Pageable pageable =
        PageRequest.of( // page arg is a index so it starts with zero
            filter.getPagination().getPageNumber() - 1,
            filter.getPagination().getPageSize(),
            Sort.by(
                Sort.Direction.valueOf(filter.getPagination().getSort().getOrder().name()),
  ...
  void should_GetSingleUserCommand_When_IdAndEmailProvided() {
    ...
    options.put("PATH", Arrays.asList("/a.txt", "/b.jpg"));
    uce.setOptions(options);
    when(userCommandRepository.findOneByIdAndUserEmail(isA(Long.class), isA(String.class)))
        .thenReturn(uce);
  ...
  private Filter generateTestFilter() {
    ...
    pagination.setPageNumber(
        1); // pageNumber field is not index in this dto class, so it starts with 1.
