...
import com.wirehall.commandhunt.backend.model.props.*;
...
public class SchemaManager {
  ...
  private static void createProperties(final JanusGraphManagement management) {
    ...
    management.makePropertyKey(FlagProperty.IS_GROUPING_ALLOWED.toLowerCase()).dataType(String.class).make();
    management.makePropertyKey(FlagProperty.IS_SOLITARY.toLowerCase()).dataType(String.class).make();
    management.makePropertyKey(FlagProperty.SEQUENCE.toLowerCase()).dataType(Byte.class).make();

    management.makePropertyKey(OptionProperty.DATA_TYPE.toLowerCase()).dataType(String.class).make();
    management.makePropertyKey(OptionProperty.IS_REPEATABLE.toLowerCase()).dataType(String.class).make();
    management.makePropertyKey(OptionProperty.IS_MANDATORY.toLowerCase()).dataType(String.class).make();
