...
public class PublicCommandMapper {
   ...
   * @param mapAssociations This flag should be set to false if entity's associations are not
   *     required. Since un-necessarily mapping this will invoke lazy loading of these associations.
   * @return The public-command dto.
   */
  public PublicCommand mapToPublicCommand(
      PublicCommandEntity publicCommandEntity, boolean mapAssociations) {
    ...
    if (mapAssociations) {
      publicCommand.setFlags(
          publicCommandEntity.getFlags().stream().collect(Collectors.toMap(f -> f, f -> true)));
   ...
   */
  public PublicCommandEntity mapToPublicCommandEntity(
      PublicCommand publicCommand, String userEmail) {
    ...
    Set<String> flags =
        publicCommand.getFlags().entrySet().stream()
