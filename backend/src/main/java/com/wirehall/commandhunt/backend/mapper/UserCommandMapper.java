...
public final class UserCommandMapper {
   ...
   */
  public UserCommand mapToUserCommand(
      UserCommandEntity userCommandEntity, boolean mapAssociations) {
    ...
    if (mapAssociations) {
      userCommand.setFlags(
          userCommandEntity.getFlags().stream().collect(Collectors.toMap(f -> f, f -> true)));
  ...
  public UserCommandEntity mapToUserCommandEntity(UserCommand userCommand, String userEmail) {
    ...
    Set<String> flags =
        userCommand.getFlags().entrySet().stream()
