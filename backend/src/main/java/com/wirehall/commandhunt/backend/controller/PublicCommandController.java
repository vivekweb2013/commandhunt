...
/**
 * Controller for performing action on public-command. A public command can be retrieved by anyone
 * without the need to login into the system. Only logged-in user can create a public command. A
 * public command can not be updated after creation. Not even the owner of the command is allowed to
 * update. A public command can be deleted by the owner.
 */
@RestController
...
public class PublicCommandController {
  ...
  public PublicCommand getPublicCommandById(
      @PathVariable(name = "id") Long publicCommandId, Principal principal) {
  ...
  public PublicCommand addPublicCommand(
      @RequestBody PublicCommand publicCommand, Principal principal) {
    LOGGER.info(
        "Request from user: {} for adding public-command: {}", principal.getName(), publicCommand);
  ...
  public void deleteUserCommand(
      @PathVariable(name = "id") Long publicCommandId, Principal principal) {
    LOGGER.info(
        "Request from user: {} for deleting public-command with id: {}",
        principal.getName(),
        publicCommandId);
